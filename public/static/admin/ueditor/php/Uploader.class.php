...
//			$this->imageWaterMark($this->filePath,9,'mark.png');
     ...
     * @return mixed
     */
    private function upFileToOss() {

        require_once $_SERVER["DOCUMENT_ROOT"]."/../vendor/autoload.php";

        $config = include_once $_SERVER["DOCUMENT_ROOT"]."/../config/oss.php";

        try {
            $ossClient = new OssClient($config["KeyId"], $config["KeySecret"], $config["Endpoint"], $config["UseCname"]);
        ...
        $ossObjectName = "image/".$this->fileName;
        $this->fullName = $config["Cname"]. "/" .$ossObjectName;
        ...
        $ossClient->uploadFile($config["Bucket"],$ossObjectName,$file['tmp_name']);
        $this->stateInfo = $this->stateMap[0];
    }
